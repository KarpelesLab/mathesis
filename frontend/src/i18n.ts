...
const messages = {
  ...
  ja: {
    ...
    guide: [
      ...
      {
        h: '式の入力',
        p: '演算子 + - * / ^ と後置の !（階乗）を使い、括弧でまとめます。関数は角括弧 Head[引数, …] で呼び出し、リストは波括弧 {a, b, c} で書きます。= で変数に代入でき（例: x = RandomPrime[1000]）、以降のセルで x を再利用できます（== は等値のまま）。% は直前の結果を指します。SMT ソルバーに渡す文字列は二重引用符 "…" で囲みます。(* … *) でコメントを書けます。',
      },
      {
        h: '厳密な値と小数表示',
