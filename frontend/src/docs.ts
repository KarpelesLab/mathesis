...
export const CATEGORIES: Category[] = [
  ...
  },
  {
    ...
    fns: [
      ...
      { name: 'Solve', syntax: 'Solve[c, vars]', examples: ['Solve[x + y == 4 && x >= 0 && y >= 0, {x, y}]', 'Solve[p || q, {p, q}]'], desc: t('All solutions over the integers (or one over the reals), as exact typeset rules.', 'Toutes les solutions sur les entiers (ou une sur les réels), en règles exactes.', '整数上の全解（実数では 1 つ）を厳密な規則で返す。') },
