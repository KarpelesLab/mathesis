...
export const CATEGORIES: Category[] = [
  ...
  },
  {
    ...
    fns: [
      { name: 'D', syntax: 'D[f, x]  ·  D[f, {x, n}]  ·  D[f, {{x, y, …}}]', examples: ['D[x^3 + x, x]', 'D[x^2 y, {{x, y}}]', 'Det[D[{(1+x y)^3 z + y^2 (1 + x y) (4 + 3x y), y + 3x (1 + x y)^2 z + 3x y^2 (4 + 3x y), 2x - 3x^2 y - x^3 z}, {{x,y,z}}]]'], desc: t('Symbolic derivative of a polynomial expression: ∂f/∂x, the n-th derivative, the gradient, or (of a list of functions) the Jacobian matrix. Free symbols are the variables; juxtaposition multiplies (2 x, x y). The third example is the map from Alpöge’s 2026 counterexample to the Jacobian conjecture: one click verifies exactly that its Jacobian determinant is the constant −2, even though the map is not injective (F(1, 0, 2) = F(0, 6, −142)).', "Dérivée symbolique d'une expression polynomiale : ∂f/∂x, la dérivée n-ième, le gradient, ou (d'une liste de fonctions) la matrice jacobienne. Les symboles libres sont les variables ; la juxtaposition multiplie (2 x, x y). Le troisième exemple est l'application du contre-exemple d'Alpöge (2026) à la conjecture jacobienne : un clic vérifie exactement que son déterminant jacobien est la constante −2, bien que l'application ne soit pas injective (F(1, 0, 2) = F(0, 6, −142)).", '多項式式の記号微分：∂f/∂x、n 階微分、勾配、または（関数のリストなら）ヤコビ行列。自由記号が変数となり、並置は乗算（2 x, x y）。3 つ目の例は Alpöge（2026）によるヤコビアン予想の反例の写像：ヤコビ行列式が定数 −2 であることをワンクリックで厳密に検証できる（それでも単射ではない：F(1, 0, 2) = F(0, 6, −142)）。') },
    ],
  },
  {
    id: 'plotting',
